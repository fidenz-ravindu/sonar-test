...
import { Atma, Barlow, Cabin, Catamaran, OpenSans, Oxygen, Roboto } from "../src/font";
...
class ArchievementsComponent extends Component {
    async getArchievements() {
        ...
        await get_archievements((res) => {
            let i = 0;
            dispatch(set({
                key: "archievements", value: res.archievements.map(elm => {
                    elm["key"] = i++;
                    return elm;
                })
            }));
            ...
            <>
                ...
                <section className={ComponentStyles.page}>
                    ...
                    <div className={Styles.container}>
                        ...
                        {archievements.map(elm => <Archievement className={ComponentStyles.fadeIn} style={{ animationDelay: `${elm.key * 0.2}s` }} key={elm.key} archievement={elm} />)}
                    </div>
                    {!isWaiting && archievements.length === 0 && (
                        <>
                            <div
                                className={[Atma.className, ComponentStyles.fadeIn].join(" ")}
                                style={{ margin: "25px", color: "wheat", fontSize: "1.2rem", textAlign: "center" }}
                            >
                                No results found.
                            </div>
                        </>
